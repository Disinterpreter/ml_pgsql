...
int CFunctions::pg_query(lua_State* luaVM)
{
    ...
    PGconn* connection = static_cast<PGconn*>(lua_touserdata(luaVM, 1));
    LUA_FUNCTION_ASSERT("pg_query", connection);

    if (PQstatus(connection) != CONNECTION_OK && PQsetnonblocking(connection, 1) == 0) {
    ...
    PGresult* query_result = PQexecParams(connection, query_str.c_str(), args.size(), NULL, (char**)args.data(), NULL, NULL, 0);
...
int CFunctions::pg_poll(lua_State* luaVM)
{
    ...
    PGresult *query_result = static_cast<PGresult*>(lua_touserdata(luaVM, 1));
